...
impl<'de> Deserialize<'de> for FieldInstance {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error> where D: Deserializer<'de> {
        ...
        let value = match helper.field_instance_type.as_str() {
            ...
            "Tile" =>
                FieldValue::Tile(
                    Option::<TilesetRectangle>
                        ::deserialize(helper.value)
                        .map_err(de::Error::custom)?
                ),
            "EntityRef" =>
                FieldValue::EntityRef(
                    Option::<ReferenceToAnEntityInstance>
                        ::deserialize(helper.value)
                        .map_err(de::Error::custom)?
                ),
            "Point" => {
                let point_helper = Option::<GridPoint>
                    ::deserialize(helper.value)
                    .map_err(de::Error::custom)?;
            ...
            "Array<Tile>" =>
                FieldValue::Tiles(
                    Vec::<Option<TilesetRectangle>>
                        ::deserialize(helper.value)
                        .map_err(de::Error::custom)?
                ),
            "Array<EntityRef>" =>
                FieldValue::EntityRefs(
                    Vec::<Option<ReferenceToAnEntityInstance>>
                        ::deserialize(helper.value)
                        .map_err(de::Error::custom)?
                ),
            "Array<Point>" => {
                let point_helpers = Vec::<Option<GridPoint>>
                    ::deserialize(helper.value)
                    .map_err(de::Error::custom)?;
            ...
            t => {
                let enum_regex = Regex::new(r"^(LocalEnum|ExternEnum)\.").expect(
                    "enum regex should be valid"
                );
                let enums_regex = Regex::new(r"^Array<(LocalEnum|ExternEnum)\.").expect(
                    "enums regex should be valid"
                );
...
pub enum FieldValue {
    ...
    #[serde(serialize_with = "serialize_points")] Points(Vec<Option<IVec2>>),
}

fn serialize_colors<S: Serializer>(colors: &[Color], serializer: S) -> Result<S::Ok, S::Error> {
    let field_values: Vec<FieldValue> = colors
        .iter()
        .map(|c| FieldValue::Color(*c))
        .collect();
...
fn serialize_points<S: Serializer>(
    ...
    let field_values: Vec<FieldValue> = points
        .iter()
        .map(|p| FieldValue::Point(*p))
        .collect();
    field_values.serialize(serializer)
}
