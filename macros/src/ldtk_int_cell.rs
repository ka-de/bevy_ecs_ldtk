    ...
    for field in fields {
        ...
        if let Some(attribute) = ldtk_int_cell {
            field_constructions.push(
                expand_ldtk_int_cell_attribute(attribute, field_name, field_type)
            );
            continue;
        }

        let from_int_grid_cell = field.attrs
            .iter()
            .find(|a| *a.path.get_ident().as_ref().unwrap() == FROM_INT_GRID_CELL_ATTRIBUTE_NAME);
        if let Some(attribute) = from_int_grid_cell {
            field_constructions.push(
                expand_from_int_grid_cell_attribute(attribute, field_name, field_type)
            );
            continue;
        }
...
fn expand_ldtk_int_cell_attribute(
    ...
    match attribute.parse_meta().expect("Cannot parse #[ldtk_int_cell] attribute") {
        syn::Meta::Path(_) => {
            quote! {
...
fn expand_from_int_grid_cell_attribute(
    ...
    match attribute.parse_meta().expect("Cannot parse #[from_int_grid_cell] attribute") {
        syn::Meta::Path(_) => {
            quote! {
